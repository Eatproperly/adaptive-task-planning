...
当重要性、最低期待或关键约束有缺口时，技能应简短复述并只问一个关键问题，等待回答；不能先给任务树再补问。资料齐全则直接进入规划，不额外索要形式确认。旧项目和模拟案例不能替用户决定当前路线。首轮计划优先展示骨架和当前任务，详细内容按需展开。

示例：用户只说“使用任务拆解，帮我发表一篇论文”，正确行为是先询问影响方案的目标或约束；不能擅自指定研究课题、创新方法或发表期限。

更新后请在实际使用技能的环境中重新加载或更新该技能副本，再用真实请求复测。本地安装与远程仓库更新不能证明另一个会话已经读取新版。可用 [回归场景](tests/behavior-cases.md) 检查行为。
